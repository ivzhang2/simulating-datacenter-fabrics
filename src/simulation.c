...
*/
void simulation(char *filename, double interarrival_time,
                double total_simulation_time) {
  FILE *fp;
  fp = fopen("results/simulation.csv", "w+");
...
int main() {
  ...
  /* 
    ...
    Path A + B + C + D = (h0, h6, l0, s0, s1, l3, h2, h4, h5) + (h1, l1, s2, l2, s3, h3)
  */
  ...
  /* 
    ...
    Path A + B + C + D = (h0, h6, l0, s0, s1, l3, h2, h4, h5) + (h1, l1, s2, l2, s3, h3)
  */
  struct simulation_runs* simruns = generate_sim_runs(packet_array, 4);
  int DEBUG = 1;
  if(DEBUG){
    ...
    for(int j = 0; j < total_num_bundles; j++){
      ...
      printf("\nprinting out SEND ID and RECEIVE ID from packet bundle #%d of size #%d \n", j, size_of_bundle[j]);
        printf("------------------------------------------\n");
        for(int i = 0; i < size_of_bundle[j]; i++){
          printf("send_id %zu, receive_id %zu\n", packets[j][i].send_id, packets[j][i].receive_id);
        }
    }
  }
