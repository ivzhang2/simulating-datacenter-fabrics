...
int main(int argc, char **argv) {
  ...
  char result_fname[255];
  memset(&result_fname, 0, 255);
  sprintf(result_fname, "results/fct.%d.csv", rank);

  if (rank < pruns->n_topoarr) {
    complete_flows(&pruns->tarr[rank], pruns->topoarr[rank]);
    export_flow_times(&pruns->tarr[rank], result_fname, strlen(result_fname));
  }
