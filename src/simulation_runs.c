...
// 	struct packet_t** parr_arr; // array of packets in same topo
// 	struct topology_t* topoarr; // topo based on packet movement
// };
	...
	for(int i = 0; i < n_path; i++){
		...
		if(type == LINK){
			if(topo->n_links == 0){
				...
				topo->links = realloc(topo->links, sizeof(struct network_object_t *) * (topo->n_links + 1));
			...
			if(topo->n_switches == 0){
				...
				topo->switches = realloc(topo->switches, sizeof(struct network_object_t *) * (topo->n_switches + 1));
			...
			if(topo->n_servers == 0){
				...
				topo->servers = realloc(topo->servers, sizeof(struct network_object_t *) * (topo->n_servers + 1));
...
struct topology_t* union_topology(struct topology_t* topo, struct network_object_t **path, size_t n_path){
	...
	for(int i = 0; i < n_path; i++){
		...
		if(type == LINK){
			...
			if(!inTopo){
				temp->links = realloc(temp->links, sizeof(struct network_object_t *) * (temp->n_links + 1));
			...
			int inTopo = is_in_topology(path[i], topo);
			if(!inTopo){
				temp->switches = realloc(temp->switches, sizeof(struct network_object_t *) * (temp->n_switches + 1));
			...
			int inTopo = is_in_topology(path[i], topo);
			if(!inTopo){
				temp->servers = realloc(temp->servers, sizeof(struct network_object_t *) * (temp->n_servers + 1));
...
/* Logic:
	...
	(I.E. Loop through the routes of the current packet and if a route matches, union the who PATH
		Else make new topology)
*/

struct simulation_runs* generate_sim_runs(struct packet_t** p_arr, size_t num_packets){
	struct simulation_runs* sim_runs = malloc(sizeof(struct simulation_runs));
	...
	int match = 0; // 1 if path was merge, else 0 to create new topology and packet array
	int count[num_packets]; // maintains the count of each packet array in packet_arr
	...
	for (int i = 0; i < num_packets; ++i){
		...
		for(int j = 0; j < num_topology; j++){
			...
			for(int k = 0; k < p_arr[i]->n_path; k++){
				if(is_in_topology(p_arr[i]->path[k], topo_arr[j])){
					topo_arr[j] = union_topology(topo_arr[j], p_arr[i]->path, p_arr[i]->n_path);
					match = 1;
					packet_arr[j] = realloc(packet_arr[j], sizeof(struct packet_t)*(count[j]+1));
		...
		if(num_topology == 0 || !match){
			packet_arr = realloc(packet_arr, sizeof(struct packet_t*)*(num_topology+1));
			topo_arr = realloc(topo_arr, sizeof(struct topology_t *)*(num_topology+1));;
			packet_arr[num_topology] = p_arr[i];
			count[i] = 1;
			topo_arr[num_topology] = create_new_topology(p_arr[i]->path, p_arr[i]->n_path);
			num_topology += 1;
		}
	}
